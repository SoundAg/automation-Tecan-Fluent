...
standards_filepath = reordered_measurement_filepaths.pop()
...
elution_plate_id_list, backdilution_plate_id_list, filename_list, well_id_list, rfu_data_list = [], [], [], [], []

for loop_count, excel_filepath in enumerate(reordered_measurement_filepaths, start=1):
    ...
    rfu_data_list.extend([float(val) for sublist in transposed_data for val in sublist])
    ...
    calculated_rna_conc_list = [(val * slope * 2) + y_intercept for val in rfu_data_list]
