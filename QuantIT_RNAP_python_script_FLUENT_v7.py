# The following code was updated from the QuantIT_RNAP_python_script_LOCAL_v5
# To run: cd C:/Users/Tecan/Desktop/Tecan Fluent desktop files/RNAP data management
# python QuantIT_RNAP_python_script_FLUENT_v5.py
...
# Step 5: Plot standards and calculate R-squared for QC purposes.
...
r_squared = r_value**2
plt.scatter(new_standards_df[x_axis_label], new_standards_df[y_axis_label], label='Standards measurements')
plt.plot(new_standards_df[x_axis_label], slope * new_standards_df[x_axis_label] + intercept, color='red', label='Line of Best Fit')
plt.text(0.9, 0.1, f'R-squared: {r_squared:.2f}', fontsize=10, color='blue', ha='right', va='bottom', transform=plt.gca().transAxes)
...
for loop_count, excel_filepath in enumerate(only_measurement_filepaths, start=1):
    ...
    if loop_count == 1:
        ...
        calculated_rna_conc_list_1 = [round(((val * slope * 2) + y_intercept),2) for val in rfu_data_list_1]
    ...
    if loop_count == 2: 
        ...
        calculated_rna_conc_list_2 = [round(((val * slope * 2) + y_intercept),2) for val in rfu_data_list_2]
    ...
    if loop_count == 3: 
        ...
        calculated_rna_conc_list_3 = [round(((val * slope * 2) + y_intercept),2) for val in rfu_data_list_3]
    ...
    if loop_count == 4: 
        ...
        calculated_rna_conc_list_4 = [round(((val * slope * 2) + y_intercept),2) for val in rfu_data_list_4]
...
# Export the Standards Plot for QC
standards_plot_filename = f"{reformatted_datetime_string}_RNA Quant standards plot.png"
gdrive_standards_plot_file_path = "G:/.shortcut-targets-by-id/1kOpNriLPL3kgZ-DM8TK4O3Bw06XKmk6M/Research/RnD Transfer/Fluent 1080/RNA Prep Data/Backdilution metadata log/"
plt.savefig(gdrive_standards_plot_file_path + standards_plot_filename)